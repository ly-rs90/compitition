...
class Choice(RequestHandler):
    def get(self, *args, **kwargs):
        con = self.get_argument('continue', False)
        count = self.get_argument('count', 0)
        start = self.get_argument('start', 0)
        try:
            total_count = self.application.db.execute('''select count(*) from choice where use=1''').fetchone()[0]
            r = self.application.db.execute('''
                select id, content from choice where use=1 limit ? offset ?
    ...
    def post(self, *args, **kwargs):
        ...
        if mode == 'train':
            ...
            try:
                sql_str = '''select content, c1, c2, c3, c4, ans from choice limit 1 offset ?'''
        ...
        if mode == 'get-count':
            ...
            try:
                count = self.application.db.execute('''
                    select count(*) from choice where content like '%{}%'
        ...
        if mode == 'get-question':
            ...
            try:
                r = self.application.db.execute('''
                    select id, content, c1, c2, c3, c4, ans, use from choice where content like '%{}%' limit 30 offset ?
        ...
        if mode == 'add':
            ...
            if not content:
                ...
                try:
                    self.application.db.execute('''
                        insert into choice (id, content, c1, c2, c3, c4, ans) values(?, ?, ?, ?, ?, ?, ?)
        ...
        if mode == 'modify':
            ...
            if not content:
                ...
                try:
                    self.application.db.execute('''
                        update choice set content=?, ans=?, c1=?, c2=?, c3=?, c4=? where id=?
        ...
        if mode == 'use-question':  # 批量启用/禁用试题
            ...
            try:
                self.application.db.execute('''
                    update choice set use=? where id in ({})
                '''.format(id_str), (use, ))
                self.application.db.commit()
            except Exception as e:
                msg.code = 1
