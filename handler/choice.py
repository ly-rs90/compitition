...
class Choice(RequestHandler):
    def get(self, *args, **kwargs):
        ...
        try:
            total_count = self.application.db.execute('''select count(*) from choice''').fetchone()[0]
            r = self.application.db.execute('''select id, content from choice limit ? offset ?''', (count, start)).fetchall()
            data = []
            for n in r:
    ...
    def post(self, *args, **kwargs):
        ...
        if mode == 'get-question':
            ...
            try:
                r = self.application.db.execute('''
                    select id, content, c1, c2, c3, c4, ans from choice where content like '%{}%' limit 30 offset ?
